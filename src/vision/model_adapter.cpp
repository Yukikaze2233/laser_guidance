...
auto unletterbox_box(
    const cv::Rect2f& bbox, const ModelImageTransform& transform, const Frame& frame)
...
auto brightness_for_bbox(const Frame& frame, const cv::Rect2f& bbox) -> float {
    ...
    const cv::Rect roi(
        static_cast<int>(std::floor(clipped->x)), static_cast<int>(std::floor(clipped->y)),
...
auto success_result(
    const Frame& frame, std::vector<DecodedDetection> detections, std::string message = {})
    -> ModelAdapterResult {
...
auto decode_raw_output(
    const Frame& frame, const ModelRunResult& run_result, const ModelTensorData& tensor)
    -> ModelAdapterResult {
    const auto rows_cols = reshape_rows_cols(tensor);
    if (!rows_cols) {
        return failure_result(
            "YOLO raw output must be [1,N,5+C] or [N,5+C] (" + format_tensor_metadata(tensor)
            + ")");
    }
    ...
    if (cols < 6) {
        return failure_result(
            "YOLO raw output last dimension must be at least 6 (" + format_tensor_metadata(tensor)
            + ")");
    }
    if (tensor_shape_element_count(tensor.shape) != rows * cols)
        return failure_result(
            "YOLO raw output size does not match its shape (" + format_tensor_metadata(tensor)
            + ")");
...
auto decode_nms_rows(
    const Frame& frame, const ModelRunResult& run_result, const ModelTensorData& tensor)
    -> ModelAdapterResult {
    const auto rows_cols = reshape_rows_cols(tensor);
    if (!rows_cols) {
        return failure_result(
            "YOLO NMS output must be [1,N,6/7] or [N,6/7] (" + format_tensor_metadata(tensor)
            + ")");
    }
    ...
    if (cols != 6 && cols != 7)
        return failure_result(
            "YOLO NMS output last dimension must be 6 or 7 (" + format_tensor_metadata(tensor)
            + ")");
...
auto decode_split_nms_outputs(const Frame& frame, const ModelRunResult& run_result)
    ...
    if (run_result.outputs.size() != 4)
        return failure_result("YOLO split NMS contract requires four output tensors");
    ...
    if (num.values.empty())
        return failure_result("YOLO split NMS num_detections output is empty");
    if (boxes.shape.size() != 3 || boxes.shape[0] != 1 || boxes.shape[2] != 4)
        return failure_result("YOLO split NMS boxes output must be [1,N,4]");
    if (scores.shape.size() != 2 && scores.shape.size() != 3)
        return failure_result("YOLO split NMS scores output must be [1,N] or [1,N,1]");
...
auto adapt_yolo_outputs(const Frame& frame, const ModelRunResult& run_result)
    ...
    if (run_result.outputs.size() == 4)
        return decode_split_nms_outputs(frame, run_result);

    if (run_result.outputs.size() != 1) {
        return failure_result(
            "model output contract is unsupported (outputs="
    ...
    if (!rows_cols) {
        return failure_result(
            "model output contract is unsupported for the primary output shape ("
            + format_tensor_metadata(tensor) + ")");
    }

    const auto [rows, cols] = *rows_cols;
    if (rows == 0 || cols == 0)
        return failure_result("model output tensor is empty");
    if (cols == 6 && !looks_like_nms_rows(tensor))
        return decode_raw_output(frame, run_result, tensor);
    if (cols == 6 || cols == 7)
        return decode_nms_rows(frame, run_result, tensor);
    if (cols > 7)
        return decode_raw_output(frame, run_result, tensor);
