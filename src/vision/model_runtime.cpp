...
auto extract_value_info(
    ...
    for (std::size_t index = 0; index < count; ++index) {
        Ort::AllocatedStringPtr name = is_input ? session.GetInputNameAllocated(index, allocator)
...
auto extract_tensor_data(const ModelValueInfo& metadata, const Ort::Value& value)
    ...
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_DOUBLE: {
        ...
        std::transform(
            data, data + element_count, tensor.values.begin(),
    ...
    default:
        throw std::runtime_error("unsupported ONNX output element type: " + tensor.element_type);
    }
...
auto ModelRuntime::run(const cv::Mat& image) const -> ModelRunResult {
    ...
    try {
        ...
        Ort::Value input_tensor = Ort::Value::CreateTensor<float>(
            memory_info, const_cast<float*>(prepared.values.data()), prepared.values.size(),
            prepared.shape.data(), prepared.shape.size());
        ...
        const auto ort_outputs = details_->session->Run(
            Ort::RunOptions{nullptr}, input_names.data(), &input_tensor, input_names.size(),
