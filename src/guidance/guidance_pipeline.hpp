...
public:
    ...
    auto process_ekf_guided(
        const cv::Point2f& ekf_center, const ModelCandidate* candidate,
        const LidarFrame* lidar_frame, float& io_depth_mm) -> std::string;
    ...
    auto estimate_depth(const ModelCandidate& candidate, const LidarFrame* lidar_frame = nullptr)
...
private:
    ...
    auto write_single(float theta_x, float theta_y, float depth_mm, const cv::Point2f& center)
        -> std::string;
    auto write_voltage_single(float x_voltage, float y_voltage, const cv::Point2f& center)
        -> std::string;
    auto process_direct_voltage_guided(
        const cv::Point2f& ekf_center, const ModelCandidate* candidate) -> std::string;
    auto update_scan_center(float theta_x, float theta_y, float depth_mm, const cv::Point2f& center)
        -> std::string;
