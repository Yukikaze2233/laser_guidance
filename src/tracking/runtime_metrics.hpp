...
[[nodiscard]] inline auto age_ms(const Clock::time_point& earlier, const Clock::time_point& later)
    -> std::int64_t {
...
struct StaleFramePolicy {
    ...
    [[nodiscard]] auto classify_before_worker(const RuntimeMetricSample& metrics) const
        -> StaleReason {
    ...
    [[nodiscard]] auto make_before_inference_sample(
        const Clock::time_point& capture_time, const Clock::time_point& worker_start_time,
        const Clock::time_point& infer_start_time) const -> RuntimeMetricLogSample {
    ...
    [[nodiscard]] auto make_after_publish_sample(
        const Clock::time_point& capture_time, const Clock::time_point& worker_start_time,
        const Clock::time_point& infer_start_time, const Clock::time_point& publish_time) const
