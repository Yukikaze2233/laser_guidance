...
auto make_observation_backend(const Config& config) -> std::unique_ptr<ObservationBackend> {
    ...
    case InferenceBackendKind::bright_spot: return std::make_unique<BrightSpotBackend>(config);
    case InferenceBackendKind::model: return std::make_unique<ModelBackend>(config);
    }
