...
auto generate_chessboard_object_points(int cols, int rows, float square_mm)
    ...
    for (int r = 0; r < rows; ++r)
        for (int c = 0; c < cols; ++c)
            points.emplace_back(
                static_cast<float>(c) * square_mm, static_cast<float>(r) * square_mm, 0.0F);
    return points;
}
...
auto try_find_chessboard(const cv::Mat& gray) -> ChessboardDetection {
    ...
    const bool found = cv::findChessboardCorners(
        gray, board, corners, cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_NORMALIZE_IMAGE);
    ...
    cv::cornerSubPix(
        gray, corners, cv::Size(kCornerRefineWin, kCornerRefineWin), cv::Size(-1, -1), criteria);

    return {.found = true, .corners = std::move(corners)};
}

auto draw_text_box(
    cv::Mat& img, std::string_view text, cv::Point org, cv::Scalar fg = kOverlayFg,
    double scale = kOverlayFontScale, int thickness = kOverlayThickness) -> void {
    int baseline = 0;
    const cv::Size text_size =
        cv::getTextSize(std::string(text), kOverlayFont, scale, thickness, &baseline);
    const cv::Rect bg_rect(
        org.x, org.y - text_size.height - baseline, text_size.width + kOverlayMargin,
    ...
    overlay.copyTo(roi);
    cv::putText(
        img, std::string(text), cv::Point(org.x + kOverlayMargin / 2, org.y - baseline / 2),
...
auto draw_status_overlay(cv::Mat& img, const CalibrationUiStatus& status) -> void {
    ...
    if (result.valid) {
        ...
        if (!result.camera_matrix.empty()) {
            ...
            draw_text_box(
                img, std::format("fx={:.1f} fy={:.1f} cx={:.1f} cy={:.1f}", fx, fy, cx, cy),
...
auto draw_key_hints(cv::Mat& img) -> void {
    ...
    for (const auto& line : {
        ...
        draw_text_box(img, line, {kOverlayMargin, y}, kOverlayFg, kHintsFontScale, kHintsThickness);
        y += kHintsLineSpacing;
    }
}

auto run_calibration(
    const std::vector<CalibSample>& samples, const std::vector<cv::Point3f>& object_template)
    -> CalibResult {
    CalibResult result;
    if (samples.empty())
        return result;
    ...
    const double rms = cv::calibrateCamera(
        all_obj, all_img, image_size, camera, dist, rvecs, tvecs, kCalibFlags, criteria);
...
auto save_calibration_yaml(const CalibResult& result, const std::filesystem::path& path)
    ...
    out << YAML::Key << "camera_matrix" << YAML::Value << YAML::Flow << YAML::BeginSeq;
...
auto load_calibration_yaml(const std::filesystem::path& path)
    ...
    try {
        ...
        result.valid = true;
        result.image_size =
            cv::Size(calib["image_width"].as<int>(), calib["image_height"].as<int>());
...
auto set_flash(std::vector<FlashMessage>& messages, std::string text, bool warn = false) -> void {
    using namespace std::chrono_literals;
    messages.push_back({std::move(text), std::chrono::steady_clock::now() + 1500ms, warn});
}

auto expire_flash(std::vector<FlashMessage>& messages) -> void {
    const auto now = std::chrono::steady_clock::now();
    std::erase_if(messages, [now](const auto& m) { return m.expiry <= now; });
}

auto draw_flash_messages(cv::Mat& img, const std::vector<FlashMessage>& messages) -> void {
    int y = kOverlayLineSpacing * 4;
    for (const auto& m : messages) {
        draw_text_box(img, m.text, {kOverlayMargin, y}, m.warn ? kOverlayWarn : kOverlayOk);
        y += kOverlayLineSpacing;
    }
}
...
public:
    ...
    auto capture(const ChessboardDetection& detection, cv::Size image_size) -> void {
        ...
        flash(
            std::format(
                "Captured sample #{} — {} corners found", samples_.size(),
                detection.corners.size()));
    }

    auto calibrate() -> void {
        if (static_cast<int>(samples_.size()) < kMinSamples) {
            flash(
                std::format("Need at least {} samples (have {})", kMinSamples, samples_.size()),
                true);
            return;
        }
        ...
        flash(std::format("Calibrated! RMS={:.3f} px — press [u] to preview", result_.rms_error));
    }
    ...
    auto load(const std::filesystem::path& output_path) -> void {
        ...
        flash(std::format("Loaded → {} (RMS={:.3f} px)", output_path.string(), result_.rms_error));
    }
...
auto handle_key(
    ...
    switch (key) {
    case 'c':
    case ' ': session.capture(detection, image_size); break;
    case 'p': session.calibrate(); break;
    case 's': session.save(output_path); break;
    case 'l': session.load(output_path); break;
    case 'u': session.toggle_undistorted(); break;
    case 'r': session.reset(); break;
    case 'h': session.toggle_hints(); break;
    default:
...
int main(int argc, char** argv) {
    try {
        ...
        std::println(
            "Camera opened: {} ({}x{} @ {} fps {})", negotiated.device_path.string(),
            negotiated.width, negotiated.height, negotiated.framerate, negotiated.fourcc);

        CalibSession session(
            generate_chessboard_object_points(kChessboardCols, kChessboardRows, kSquareSizeMm));
        ...
        while (true) {
            ...
            if (session.show_undistorted() && session.result().valid)
                cv::undistort(
                    display, display, session.result().camera_matrix, session.result().dist_coeffs);
            ...
            if (detection.found)
                cv::drawChessboardCorners(
                    display, cv::Size(kChessboardCols, kChessboardRows), detection.corners,
                    detection.found);

            draw_status_overlay(display, session.status());
            if (session.show_hints())
                draw_key_hints(display);
            ...
            if (handle_key(key, session, detection, display.size(), output_path) == KeyAction::exit)
                break;
        }
