...
auto inspect_video(const std::filesystem::path& path) -> VideoInspection {
    ...
    if (!capture.isOpened())
        throw std::runtime_error("failed to open recorded test video");
...
int main() {
    try {
        ...
        require(
            std::filesystem::exists(recorder.metadata_path()),
        ...
        require(
            recorded_metadata.session_id == metadata.session_id,
            "recorded session metadata session_id mismatch");
        require(
            recorded_metadata.relative_video_path == metadata.relative_video_path,
            "recorded session metadata video path mismatch");
        require(
            recorded_metadata.duration_ms == metadata.duration_ms,
        ...
        require(
            recorded_metadata.height == metadata.height,
            "recorded session metadata height mismatch");
        require(
            recorded_metadata.lighting_tag == metadata.lighting_tag,
        ...
        require(
            transcoded_encoding.codec_tag_string == "avc1",
        ...
        for (const auto& frame : exported) {
            require(
                std::filesystem::exists(dataset_root / frame.relative_image_path),
