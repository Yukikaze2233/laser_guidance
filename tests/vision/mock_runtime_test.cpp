...
auto wait_until(auto predicate, const std::chrono::milliseconds timeout, const char* message)
    -> void {
...
int main() {
    try {
...

        {
            ...
            runtime.start();
            wait_until(
                [&] { return runtime.snapshot().capture_thread_exited; },
                std::chrono::milliseconds(500),
                "capture thread should finish bounded synthetic run");
            wait_until(
                [&] { return runtime.snapshot().capture_overwrites > 0; },
                std::chrono::milliseconds(500),
                "fast capture should overwrite the latest-frame slot");
            runtime.stop();
            ...
            require(
                snapshot.captured_frames == 80,
                "capture should produce the configured synthetic frame count");
            require(
                snapshot.max_capture_queue_depth <= 1,
                "latest-frame handoff should stay capacity one");
            require(
                snapshot.capture_overwrites > 0,
                "producer outrunning worker should drop old frames by overwrite");
            require(
                snapshot.published_observations > 0,
                "worker should publish some fresh observations");
            require(
                snapshot.published_observations < snapshot.captured_frames,
                "worker should consume latest frames, not build a backlog");
            require(
                snapshot.hit_state == HitState::Confirmed,
                "purple sequence should drive HIT confirmation");
        }

        {
            ...
            runtime.start();
            wait_until(
                [&] { return runtime.snapshot().stale_before_inference_drops > 0; },
                std::chrono::milliseconds(500),
                "worker should drop inputs that are stale before inference");
            runtime.stop();

            const auto snapshot = runtime.snapshot();
            require(
                snapshot.worker_thread_exited, "worker should exit after stale-drop scenario stop");
            require(
                snapshot.stale_before_inference_drops > 0,
                "stale frames should be dropped before inference");
            require(
                snapshot.published_observations == 0,
                "stale-before policy should prevent publishing old inputs");
            require(
                snapshot.capture_overwrites > 0,
                "overload should be represented as overwrites, not backlog");
            require(
                snapshot.max_capture_queue_depth <= 1,
                "stale overload should still keep queue capacity one");
        }

        {
            ...
            runtime.start();
            wait_until(
                [&] { return runtime.snapshot().stale_before_worker_drops > 0; },
                std::chrono::milliseconds(500),
                "worker should drop frames already stale at worker start");
            runtime.stop();

            const auto snapshot = runtime.snapshot();
            require(
                snapshot.stale_before_worker_drops > 0,
...

        {
            ...
            runtime.start();
            wait_until(
                [&] { return runtime.snapshot().published_observations >= 20; },
                std::chrono::milliseconds(500),
                "worker should keep publishing while debug output is overloaded");
            runtime.stop();

            const auto snapshot = runtime.snapshot();
            require(
                snapshot.debug_recording_outputs == snapshot.published_observations,
                "every published observation should attempt debug/recording output");
            require(
                snapshot.debug_recording_overwrites > 0,
                "undrained debug/recording output should overwrite instead of blocking worker");
            require(
                snapshot.published_observations >= 20,
                "debug/recording overload should not prevent worker progress");

            MockRuntimeObservation newest_observation;
            require(
                runtime.try_pop_debug_recording(newest_observation),
                "debug/recording sink should retain the latest observation");
            require(
                newest_observation.sequence == snapshot.published_observations,
...

        {
            ...
            runtime.start();
            wait_until(
                [&] { return runtime.snapshot().worker_popped_frames > 0; },
...

        {
            ...
            const auto snapshot = runtime.snapshot();
            require(
                snapshot.worker_thread_exited,
                "shutdown should unblock worker waiting on empty queue");
