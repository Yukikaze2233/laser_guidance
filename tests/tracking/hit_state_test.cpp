...
int main() {
    try {
...

        {
            ...
            require(
                machine.update(true) == HitState::Candidate, "first purple should enter Candidate");
            require(
                machine.state() == HitState::Candidate,
                "state should remain Candidate after first purple");
            require(machine.consecutive() == 1, "consecutive should count first purple");

            require(
                machine.update(true) == HitState::Candidate, "second purple should stay Candidate");
            ...
            require(
                machine.state() == HitState::Confirmed,
                "state should be Confirmed after three purple frames");
...

        {
            ...
            for (int i = 0; i < 4; ++i) {
                require(
                    machine.update(false) == HitState::Confirmed,
                    "release should not happen before five non-purple frames");
            }

            require(
                machine.consecutive() == 4,
                "consecutive should count non-purple streak during hysteresis");
            require(
                machine.update(false) == HitState::None, "fifth non-purple should release to None");
...

        {
            ...
            require(
                machine.state() == HitState::Candidate,
                "setup should be Candidate before flicker test");

            require(
                machine.update(false) == HitState::Candidate,
                "single non-purple flicker should not reset immediately");
            require(
                machine.state() == HitState::Candidate, "candidate should survive a short flicker");
            require(
                machine.consecutive() == 1,
                "consecutive should track the non-purple flicker streak");

            require(
                machine.update(true) == HitState::Candidate,
                "purple returning after a short flicker should still be tracked");
            require(machine.consecutive() == 1, "purple streak should restart after flicker");
            require(
                machine.update(true) == HitState::Candidate,
                "second purple after flicker should still be Candidate");
            require(
                machine.update(true) == HitState::Confirmed,
                "third purple after flicker should confirm");
        }
