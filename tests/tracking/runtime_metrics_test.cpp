...
int main() {
    try {
        ...
        const auto ms = [](const std::int64_t value) { return std::chrono::milliseconds(value); };
...

        {
            ...
            require(
                policy.classify_before_inference(fresh_metrics) == StaleReason::none,
                "input age at the boundary should stay fresh");
            require(
                policy.classify_before_worker(fresh_metrics) == StaleReason::none,
                "worker-start input age at the boundary should stay fresh");
            require(
                policy.classify_after_publish(fresh_metrics) == StaleReason::none,
            ...
            require(
                policy.classify_before_worker(stale_worker_metrics)
            ...
            require(
                policy.classify_before_inference(stale_input_metrics)
            ...
            require(
                policy.classify_after_publish(stale_observation_metrics)
                    == StaleReason::too_old_after_publish,
                "observation age beyond the boundary should be dropped after publish");

            const RuntimeMetricLogSample before_sample =
                policy.make_before_inference_sample(base, base + ms(2), base + ms(27));
            require(
                before_sample.metrics.input_age_at_worker_start_ms == 2,
                "before-inference sample should preserve worker-start age calculation");
            require(
                before_sample.metrics.input_age_at_infer_start_ms == 27,
                "before-inference sample should preserve infer-start age calculation");
            require(
                before_sample.stale_reason == StaleReason::too_old_before_inference,
                "before-inference stale reason should be specific to input age");

            const RuntimeMetricLogSample before_worker_sample =
                policy.make_before_inference_sample(base, base + ms(26), base + ms(27));
            require(
                before_worker_sample.stale_reason == StaleReason::too_old_before_worker,
                "before-worker stale reason should take priority when worker-start age is already "
                "stale");

            const RuntimeMetricLogSample after_sample =
                policy.make_after_publish_sample(base, base + ms(3), base + ms(14), base + ms(36));
            require(
                after_sample.metrics.observation_age_ms == 36,
                "after-publish sample should preserve observation age calculation");
            require(
                after_sample.metrics.input_age_at_worker_start_ms == 3,
                "after-publish sample should preserve worker-start age calculation");
            require(
                after_sample.metrics.input_age_at_infer_start_ms == 14,
                "after-publish sample should preserve infer-start age calculation");
            require(
                after_sample.stale_reason == StaleReason::too_old_after_publish,
...

        {
            StaleFramePolicy policy;
            const auto sample =
                policy.make_after_publish_sample(base, base, base + ms(10), base + ms(15));
            require(
                sample.stale_reason == StaleReason::none,
                "fresh frames should not be marked stale");
            require(
                sample.metrics.input_age_at_worker_start_ms == 0,
                "fresh sample worker-start age mismatch");
            require(
                sample.metrics.input_age_at_infer_start_ms == 10,
                "fresh sample infer-start age mismatch");
            require(
                sample.metrics.observation_age_ms == 15, "fresh sample observation age mismatch");
        }
